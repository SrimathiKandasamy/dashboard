...
let result=[]; 
	result = JSON.parse(localStorage.getItem("proj1"));
	var i;
	console.log(result);
	if(result == null){
		i = 1;
...
function loadFromLocal(){
	if(localStorage.getItem("proj1") != undefined && localStorage.getItem("proj1") !=""){
		result = JSON.parse(localStorage.getItem("proj1"));
	} else {
		result = [];
	}
...
function saveLocal(json){
	localStorage.setItem("proj1", JSON.stringify(json));
}
loadFromLocal();
LoadData(result);